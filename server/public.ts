...
export function mountPublic(app: express.Express, db: DB, auth: express.RequestHandler) {
  const published = async (slug: string) => {
    const [p] = await db.query("SELECT * FROM projects WHERE slug=$1 AND published_game IS NOT NULL AND published_id IS NOT NULL AND publication_status='active'", [slug]);
  ...
  app.get('/g/:slug', async (req, res) => {
    ...
    const metadata = `<title>${escape(game.title)} · game-gift</title><meta property="og:title" content="${escape(game.title)}"><meta property="og:description" content="${escape(game.description || 'A personalized playable gift, made just for you.')}"><meta property="og:url" content="${escape(url)}"><meta property="og:type" content="website"><meta property="og:image" content="${escape(origin)}/social-card.png"><meta name="twitter:card" content="summary_large_image"><link rel="canonical" href="${escape(url)}">`;
    const html = (await readFile('dist/index.html', 'utf8')).replace(/<title>.*?<\/title>/s, '').replace('</head>', `${metadata}</head>`);
    res.type('html').set('Cache-Control', 'no-store').send(html);
  });
  app.post('/api/public-games/:slug/report', async (req, res) => {
    const input = z.object({ reason: z.enum(['inappropriate_content', 'harassment', 'copyright', 'privacy', 'other']), details: z.string().max(2000).default('') }).parse(req.body);
    const p = await published(String(req.params.slug));
    await db.query('INSERT INTO reports(id,project_id,reason,details,created_at) VALUES ($1,$2,$3,$4,$5)', [randomUUID(), p.id, input.reason, input.details, new Date().toISOString()]);
    res.status(201).json({ ok: true });
  });
  app.use('/api/admin', auth, (_req, res, next) => {
    if (!(process.env.ADMIN_USER_IDS || '').split(',').includes(res.locals.user.id)) return next(fail(403, 'Operator access required'));
    next();
  });
  app.get('/api/admin/reports', async (_req, res) => res.json({ reports: await db.query('SELECT reports.*,projects.slug,projects.publication_status FROM reports LEFT JOIN projects ON reports.project_id=projects.id ORDER BY reports.created_at DESC LIMIT 100') }));
  app.put('/api/admin/projects/:id/status', async (req, res) => {
    const { status } = z.object({ status: z.enum(['active', 'disabled']) }).parse(req.body);
    const rows = await db.query('UPDATE projects SET publication_status=$1 WHERE id=$2 RETURNING id', [status, req.params.id]);
