...
export async function deliverEmails(db: DB) {
  ...
  const messages = await db.query('SELECT email_outbox.*,users.email FROM email_outbox JOIN users ON users.id=email_outbox.user_id WHERE sent_at IS NULL AND attempts<8 ORDER BY created_at LIMIT 10');
  for (const message of messages) {
    ...
    await db.query('UPDATE email_outbox SET sent_at=$1 WHERE id=$2', [new Date().toISOString(), message.id]);
  }
}
