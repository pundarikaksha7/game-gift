...
export function validateEnvironment(env: NodeJS.ProcessEnv = process.env) {
  ...
  if (env.AUTH_PROVIDER && !['legacy', 'supabase'].includes(env.AUTH_PROVIDER)) throw new Error('Invalid AUTH_PROVIDER');
  if (env.AUTH_PROVIDER === 'supabase' && (!env.SUPABASE_URL || !env.SUPABASE_ANON_KEY || !env.SUPABASE_SERVICE_ROLE_KEY)) throw new Error('Supabase Auth requires URL, anon key and service role key');
  if (env.PUBLISH_PRICE_AMOUNT && (!/^\d+$/.test(env.PUBLISH_PRICE_AMOUNT) || Number(env.PUBLISH_PRICE_AMOUNT) < 100 || Number(env.PUBLISH_PRICE_AMOUNT) > 10000000)) throw new Error('Invalid PUBLISH_PRICE_AMOUNT');
  if (env.NODE_ENV !== 'production') return;
  if (env.AUTH_PROVIDER !== 'supabase') throw new Error('Production requires Supabase Auth and private Storage');
  if (!env.SUPABASE_URL || !env.SUPABASE_SERVICE_ROLE_KEY) throw new Error('Production requires private Storage');
  ...
  for (const origin of (env.CORS_ORIGINS || env.APP_ORIGIN || '').split(',')) {
    ...
    if (parsed.protocol !== 'https:' || parsed.origin !== origin.trim()) throw new Error('CORS_ORIGINS requires exact HTTPS origins');
  }
}
...
export function trustedOrigin(origin: string, env: NodeJS.ProcessEnv = process.env) {
  if ([env.APP_ORIGIN || 'http://localhost:5173', ...(env.CORS_ORIGINS || '').split(',').map(s => s.trim())].includes(origin)) return true;
