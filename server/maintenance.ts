...
export async function maintain(db: DB) {
  ...
  for (const { id } of await db.query('SELECT id FROM deleted_accounts WHERE processed_at IS NULL LIMIT 10')) {
    await removeSupabaseAccount(id);
    await db.query('UPDATE deleted_accounts SET processed_at=$1 WHERE id=$2', [new Date().toISOString(), id]);
  }
