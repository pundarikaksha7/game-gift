...
async function run(mode) {
  ...
  const updateMobileMode = () => {
    document.body.classList.toggle(
      'mobile-landscape',
      isTouchDevice && mobileLandscapeQuery.matches,
    );
  };
  updateMobileMode();
  if (mobileLandscapeQuery.addEventListener)
    mobileLandscapeQuery.addEventListener('change', updateMobileMode);
  ...
  rotateOverlay.innerHTML =
    '<div class="rotate-phone-card"><div class="rotate-phone-icon">📱</div><h2 class="rotate-phone-title">Turn it sideways</h2><p class="rotate-phone-copy">Adventure is made for landscape on phones. Rotate your phone for the full arcade controls 🎮</p></div>';
  ...
  function getLevelLength(index = currentLevelIndex) {
    ...
    const scaledLength =
      BASE_LEVEL_LENGTH * Math.max(0.6, globalMultiplier) * Math.max(0.6, localMultiplier);
    const routeEnd = Math.max(
      ...(level.platforms || []).map((platform) => Number(platform.x) + Number(platform.w ?? 280)),
      3200,
    );
  ...
  function generateGroundHoles() {
      ...
      .flatMap((h, i) => [
        { x: h.x - 150, y: groundY - 130, w: 190, h: 24 },
        {
          ...
          motion: {
            axis: 'horizontal',
            amplitude: 150,
            period: 4.8 - currentLevelIndex * 0.4,
            phase: i,
          },
        },
    ...
    if (levelLength + 2600 - segmentStart > 40) {
      groundSegments.push({
        x: segmentStart,
        y: groundY,
        w: levelLength + 2600 - segmentStart,
        h: 400,
        isGround: true,
      });
    }
  ...
  function findSafeRespawnX(preferredX, entityWidth = PLAYER_W) {
    ...
    if (!safeSegments.length)
      return Math.max(24, Math.min(getLevelLength() - entityWidth - 24, preferredX));
    const containing = safeSegments.find(
      (segment) => preferredX >= segment.x && preferredX + entityWidth <= segment.x + segment.w,
    );
    if (containing)
      return Math.max(
        containing.x + 24,
        Math.min(containing.x + containing.w - entityWidth - 24, preferredX),
      );
    const segment = safeSegments
      .slice()
      .sort(
        (a, b) => Math.abs(a.x + a.w / 2 - preferredX) - Math.abs(b.x + b.w / 2 - preferredX),
      )[0];
    return Math.max(
      segment.x + 24,
      Math.min(segment.x + segment.w - entityWidth - 24, segment.x + 40),
    );
  }
  ...
  function getSafeGroundX(preferredX, entityWidth = PLAYER_W) {
    ...
    const containing = validSegments.find(
      (segment) => center >= segment.x + 24 && center <= segment.x + segment.w - 24,
    );
    const segment =
      containing ||
      validSegments.slice().sort((a, b) => {
    ...
    return Math.max(
      segment.x + 24,
      Math.min(segment.x + segment.w - entityWidth - 24, Number(preferredX) || segment.x + 24),
    );
  }
  ...
  const powerupDefinitions = {
    companion: {
      name: 'Companion beacon',
      duration: cfg.mechanics.helperDuration,
      color: '#ffb5de',
      description: 'Your companions join the fight',
    },
    beam: {
      name: 'Energy core',
      duration: cfg.mechanics.beamDuration,
      color: '#75e7ff',
      description: 'Hold J / K for energy beams',
    },
    boost: {
      name: 'Power boost',
  ...
  function registerStyle(style, enemy) {
    ...
    lastStyle = style;
    styleTimer = 2.5;
    const multiplier = 1 + (styleChain - 1) * 0.1;
    ...
    combatPopups.push({
      ...
      text:
        style === 'stomp'
          ? 'BOUNCE!'
          : styleChain > 1
            ? `${styleChain} HIT MIX · ×${multiplier.toFixed(1)}`
            : 'HIT!',
    });
    return multiplier;
  }
  function drawCombatPopups() {
    ...
    for (const item of combatPopups) {
      ctx.globalAlpha = Math.min(1, item.life * 3);
      ctx.fillStyle = '#ffeba0';
      ctx.fillText(item.text, item.x - cameraX, item.y - (0.8 - item.life) * 45);
    }
    ctx.restore();
  }
  ...
  function spawnEnemies() {
    ...
    const dropTargetCount = cfg.mechanics.healthDrops
      ? Math.min(count, Math.max(1, Math.min(3, Math.floor(count / 3))))
      : 0;
    ...
    for (let i = 0; i < count; i++) {
      ...
      const rawX = Math.min(
        levelLength - spec.w - 180,
        startX + progress * Math.max(520, levelLength - startX - 420) + laneOffset,
      );
      const routeIndex = Math.min(
        routePlatforms.length - 1,
        Math.round(progress * (routePlatforms.length - 1)),
      );
      const routePlatform = i % 3 === 0 ? null : routePlatforms[routeIndex];
      const x = routePlatform
        ? Math.max(
            routePlatform.x + 12,
            Math.min(routePlatform.x + routePlatform.w - spec.w - 12, rawX),
          )
      ...
      enemies.push({
        ...
        awakened: false,
        y: routePlatform
          ? routePlatform.y - (spec.h + 16)
          : Number(level.groundY ?? cfg.level.groundY) - (spec.h + 16),
        w: spec.w,
        h: spec.h,
        vx: 0,
        vy: 0,
        ...
        animTime: i * 0.3,
        iq: level.enemyIq === 'high' ? 0.95 : 0.2,
        pitTimer: 0,
      });
    }
    if (level.boss?.enabled) {
      ...
      boss = {
        ...
        defeatTimer: 0,
        animTime: 0,
        type: level.boss.characterId || Object.keys(enemyTypes)[0],
        id: 'final-boss',
        boss: true,
        x: end - 330,
  ...
  function addDefeatBurst(enemy) {
    defeatBursts.push({
      ...
      time: 0,
      duration: 0.42,
      color: enemyTypes[enemy.type].color,
    });
  }
  ...
  function damageEnemy(enemy, amount, style = null) {
    ...
    enemy.health -=
      amount * (enemy.boss && enemy.phase !== 'recover' ? getLevelConfig().boss.armor : 1);
    enemy.hurtTimer = 0.12;
    enemy.flashTimer = 0.16;
    if (enemy.health <= 0) {
      enemy.health = 0;
      stageScore += enemy.boss
        ? 1000
        : enemyTypes[enemy.type].archetype === 'large'
          ? 150
          : enemyTypes[enemy.type].archetype === 'medium'
            ? 100
            : 60;
      if (enemy.boss) {
        bossDefeated = true;
      ...
      if (
        !enemy.boss &&
        getLevelConfig().powerup !== 'none' &&
        levelDropCount < cfg.mechanics.powerupLimit &&
        Math.random() < cfg.mechanics.powerupChance
      ) {
        const kind =
  ...
  const attackDefinitions = {
    ...
    beam: {
      ...
      activeStart: 0.06,
      activeEnd: 0.27,
      range: cfg.mechanics.beamRange,
      height: 64,
      damage: cfg.mechanics.beamDamage,
      cooldown: 0.55,
    },
  };
  ...
  function getAttackHitbox() {
    ...
    const x = player.facingRight ? player.x + player.w - 6 : player.x + 6 - reach;
    return {
      x,
      ...
      damage:
        data.damage *
        (player.blueBoost && player.attack.type !== 'beam' ? cfg.mechanics.boostMultiplier : 1),
  ...
  if (mode === 'edit') {
    canvas.addEventListener('mousedown', (e) => {
      ...
      const editPlatform = editLevel.platforms?.[0] || {
        x: cfg.level.platformX,
        y: cfg.level.platformY,
        w: cfg.level.platformW,
      };
  ...
  function drawPlatforms() {
    ...
    for (const p of platforms.filter((platform) => !platform.isGround)) {
      ...
      if (!p.moving) {
        ...
        const supportColor =
          level.theme === 'midnight' ? '#382344' : level.theme === 'sunset' ? '#40566a' : '#7a431c';
      ...
      ctx.fillStyle =
        level.theme === 'midnight' ? '#5e3979' : level.theme === 'sunset' ? '#58738d' : '#b5651d';
      ctx.fillRect(px, p.y, p.w, p.h);
      ctx.strokeStyle =
        level.theme === 'midnight' ? '#d95bc5' : level.theme === 'sunset' ? '#b9d5e8' : '#8B4513';
      ctx.lineWidth = 1;
      ctx.strokeRect(px, p.y, p.w, p.h);
      ctx.fillStyle =
        level.theme === 'midnight' ? '#ff83d2' : level.theme === 'sunset' ? '#d8eef7' : '#cd853f';
  ...
  function drawEnemyFallback(enemy, sx, sy, spec) {
    ...
    ctx.arc(
      cx + 2,
      sy + 18,
      enemyTypes[enemy.type].archetype === 'large' ? 18 : 13,
      0,
      Math.PI * 2,
    );
    ctx.fill();
    ctx.fillStyle =
      enemyTypes[enemy.type].archetype === 'small'
        ? '#e45c52'
        : enemyTypes[enemy.type].archetype === 'medium'
          ? '#2a9d8f'
          : '#563267';
    ctx.fillRect(
      ...
      sy + 30,
      enemy.w - 14,
      enemyTypes[enemy.type].archetype === 'large' ? 40 : 30,
    );
    ctx.fillStyle = enemyTypes[enemy.type].archetype === 'large' ? '#3b3542' : '#4b4b62';
    ctx.fillRect(
      sx + 9,
      sy + (enemyTypes[enemy.type].archetype === 'large' ? 70 : 60),
      13,
      enemy.h - (enemyTypes[enemy.type].archetype === 'large' ? 70 : 60),
    );
    ctx.fillRect(
      sx + enemy.w - 22,
      sy + (enemyTypes[enemy.type].archetype === 'large' ? 70 : 60),
  ...
  function drawEnemyAttackEffect(enemy, sx, sy) {
    ...
    const progress = Math.max(0, 1 - enemy.attackPulse / 0.16);
  ...
  function drawEnemies() {
    for (const enemy of enemies) {
      ...
      const state = animatedArt(
        enemy.type,
        enemyImageStates[enemy.type],
        jumping ? 'jump' : running ? 'run' : 'idle',
      );
      const image = state && state.image;
      ...
      const motionBob = jumping
        ? -4
        : running
          ? -Math.abs(runPhase) * 3
          : Math.sin(enemy.animTime * 3) * 1.5;
      const motionTilt = defeated
        ? -0.55 * Math.min(1, (0.42 - enemy.defeatTimer) / 0.42)
        : enemy.boss && enemy.phase.endsWith('windup')
          ? -0.13
          : enemy.boss && enemy.phase === 'slam'
            ? 0.24
            : jumping
              ? enemy.vx >= 0
                ? 0.06
      ...
      if (state && state.ready && image) {
        ...
        ctx.drawImage(
          ...
          sy + enemy.h + 16 - spriteH + motionBob,
          spriteW,
          spriteH,
        );
      } else {
      ...
      ctx.restore();
      if (!defeated && running)
        drawRunDust(sx + enemy.w / 2, sy + enemy.h - 3, runPhase, spec.color);
      ...
      if (!defeated && cfg.mechanics.healthBars) {
        ...
        ctx.fillRect(
          barX + 1,
          barY + 1,
          (barW - 2) * Math.max(0, enemy.health / enemy.maxHealth),
          5,
        );
        if (mode === 'edit') {
  ...
  function dropPowerup(kind, enemy) {
    const platform = getPlatforms()
      .filter(
        (p) =>
          enemy.x + enemy.w / 2 >= p.x &&
          enemy.x + enemy.w / 2 <= p.x + p.w &&
          p.y >= enemy.y + enemy.h - 8,
      )
      .sort((a, b) => a.y - b.y)[0];
    const x = platform
      ? Math.max(platform.x, Math.min(platform.x + platform.w - 80, enemy.x))
      : findSafeRespawnX(enemy.x, 80);
    powerups.push({
      kind,
      x,
      y: (platform?.y ?? Number(getLevelConfig().groundY)) - 100,
      w: 80,
      h: 84,
      time: 0,
    });
  ...
  function updatePowerups(dt) {
    ...
    for (const item of powerups) {
      item.time += dt;
      if (item.time >= cfg.mechanics.pickupDelay && intersects(item, player)) {
        activatePowerup(item.kind);
        item.collected = true;
      }
    }
  ...
  function drawPowerups() {
    for (const item of powerups) {
      const sx = item.x - cameraX,
        sy = item.y + Math.sin(item.time * 4) * 7;
      const info = powerupDefinitions[item.kind],
        ready = item.time >= cfg.mechanics.pickupDelay;
      ctx.save();
      ...
      if (customArt?.ready && customArt.image) {
        ...
        ctx.fillStyle = '#faf1cf';
        ctx.fillRect(-25, -32, 51, 66);
        ctx.fillStyle = '#164c76';
        ctx.fillRect(-29, -36, 50, 66);
        ctx.fillStyle = '#e8bc65';
        ctx.fillRect(-29, -36, 7, 66);
        ctx.fillRect(-17, -26, 32, 3);
        ...
        ctx.lineTo(-9, 20);
        ctx.stroke();
        ctx.fillStyle = '#baff65';
        ctx.beginPath();
        ctx.arc(19, -27, 15, Math.PI, Math.PI * 2);
        ctx.closePath();
        ctx.fill();
    ...
    for (const ally of companions) {
      ...
      if (art.ready) {
        ...
        ctx.beginPath();
        ctx.ellipse(27, 94, 29, 6, 0, 0, Math.PI * 2);
        ctx.fill();
        ...
        for (const side of [-1, 1]) {
          ctx.beginPath();
          ctx.moveTo(27 + side * 9, 62);
          ctx.lineTo(27 + side * 12 + stride * side, 90);
          ctx.stroke();
        }
        ctx.fillStyle = color;
        ctx.beginPath();
        ctx.roundRect(9, 31, 37, 35, 9);
        ctx.fill();
        ctx.fillStyle = '#302338';
        ctx.beginPath();
        ctx.ellipse(27, 18, 20, 24, 0, 0, Math.PI * 2);
        ctx.fill();
        ctx.fillStyle = '#eab593';
        ctx.beginPath();
        ctx.ellipse(27, 21, 13, 16, 0, 0, Math.PI * 2);
        ctx.fill();
        ctx.fillStyle = '#302338';
        ctx.beginPath();
        ctx.ellipse(23, 7, 17, 9, -0.25, 0, Math.PI * 2);
        ctx.fill();
        ctx.fillStyle = '#20243b';
        ctx.fillRect(20, 19, 3, 3);
        ctx.fillRect(31, 19, 3, 3);
        ctx.strokeStyle = '#eab593';
        ctx.lineWidth = 8;
        ctx.beginPath();
        ctx.moveTo(13, 40);
        ctx.lineTo(3, 59);
      ...
      if (ally.flash > 0 && ally.target) {
        ctx.strokeStyle = color;
        ctx.lineWidth = ally.size === 'big' ? 7 : 3;
        ctx.beginPath();
        ctx.moveTo(sx + 54, sy + 49);
        ctx.lineTo(ally.target.x - cameraX, ally.target.y);
        ctx.stroke();
      }
    }
  }
  ...
  function updateSpecialEvent(dt) {
    ...
    for (const enemy of enemies) {
      if (
        ...
        enemy.health <= 0 ||
        enemy.x + enemy.w < previousX ||
        enemy.x > event.x + event.w
      )
        continue;
      event.hitIds.add(enemy.id);
      if (enemy.boss || enemyTypes[enemy.type].archetype === 'large')
        damageEnemy(enemy, Math.ceil(enemy.maxHealth * 0.52));
      else {
        enemy.health = 0;
  ...
  function drawAttackEffect(px, drawY) {
    ...
    const active =
      player.attack.elapsed >= data.activeStart && player.attack.elapsed <= data.activeEnd;
    ...
    if (player.attack.type === 'beam') {
      ...
      ctx.strokeStyle = 'rgba(72,232,255,.65)';
      ctx.lineWidth = 16;
      ctx.beginPath();
      ctx.moveTo(beamX, beamY);
      ctx.lineTo(beamX + reach * 1.4, beamY);
      ctx.stroke();
      ctx.fillStyle = '#dfffff';
      ctx.font = '700 20px system-ui';
      ctx.textAlign = 'left';
      for (let i = 0; i < 5; i++)
        ctx.fillText(['₹', '7', '4', '%', '∞'][i], beamX + 30 + i * 68, beamY - 10 - (i % 2) * 18);
    ...
    if (active) {
      ctx.strokeStyle = player.blueBoost
        ? player.attack.type === 'kick'
          ? '#ff43df'
          : '#32ffc5'
        : 'rgba(255, 241, 168, 0.75)';
      ctx.lineWidth = 2;
      ctx.setLineDash([5, 4]);
  ...
  function drawPlayer() {
    const motion =
      !player.onGround && !player.attack
        ? 'jump'
        : player.animState === 'moving' && Math.abs(player.vx) > 5
          ? 'run'
          : 'idle';
    ...
    const motionTilt = player.attack
      ? player.facingRight
        ? 0.045
        : -0.045
      : jumping
    ...
    if (heroArt.ready) {
      ...
      ctx.drawImage(
        heroImage,
        drawX,
        drawY + player.h + 16 - spriteH + motionBob,
        spriteW,
        spriteH,
      );
    } else {
  ...
  function gameLoop(timestamp) {
    ...
    const targetCamX = Math.max(
      0,
      Math.min(getLevelLength() - viewportWidth(), player.x - viewportWidth() / 2 + player.w / 2),
    );
    cameraX =
    ...
    if (mode === 'preview' && cfg.grid) {
      ...
      for (let x = -(cameraX % unit); x < viewportWidth(); x += unit) {
        ctx.beginPath();
        ctx.moveTo(x, 0);
        ctx.lineTo(x, viewportHeight());
        ctx.stroke();
      }
      for (let y = 0; y < viewportHeight(); y += unit) {
        ctx.beginPath();
        ctx.moveTo(0, y);
  ...
  function updateBoss(enemy, dt) {
    const ground = Number(getLevelConfig().groundY),
      left = getLevelLength() - 900,
      right = getLevelLength() - 220;
    ...
    if (enemy.phase === 'approach') {
      ...
      enemy.vx =
        Math.abs(player.x + player.w / 2 - enemy.x - enemy.w / 2) > (enemy.w + player.w) / 2 + 20
          ? (enemy.facingRight ? 1 : -1) * (enraged ? 180 : 125)
          : 0;
      ...
      if (enemy.timer <= 0) {
        ...
        enemy.timer = enraged ? 0.75 : 1.05;
        enemy.vx = 0;
        enemy.attackHitDone = false;
      }
    } else if (enemy.phase.endsWith('windup')) {
      if (enemy.timer <= 0) {
        enemy.phase = enemy.phase.startsWith('charge') ? 'charge' : 'slam';
        enemy.timer = enemy.phase === 'charge' ? 0.7 : 0.28;
      }
      ...
      if (
        !enemy.attackHitDone &&
        intersects({ x: enemy.x, y: ground - 88, w: enemy.w, h: 88 }, player)
      ) {
        hurtPlayer(getLevelConfig().boss.damage, enemy.x);
        enemy.attackHitDone = true;
      }
      if (enemy.timer <= 0) {
        enemy.phase = 'recover';
        enemy.timer = enraged ? 1.1 : 1.65;
        enemy.vx = 0;
      }
    } else if (enemy.phase === 'slam') {
      if (
        !enemy.attackHitDone &&
        Math.abs(player.x - enemy.x) < 330 &&
        player.y + player.h > ground - 55
      ) {
        hurtPlayer(getLevelConfig().boss.damage * 1.25, enemy.x);
        enemy.attackHitDone = true;
      }
      if (enemy.timer <= 0) {
        enemy.phase = 'recover';
  ...
  function drawBossWarning() {
    ...
    const sx = boss.x - cameraX,
      ground = Number(getLevelConfig().groundY);
    ...
    const label =
      boss.phase === 'charge-windup'
        ? 'SHOULDER RUSH · JUMP!'
        : boss.phase === 'slam-windup'
          ? 'GROUND SLAM · GET AIRBORNE!'
          : boss.phase === 'recover'
            ? 'EXPOSED · STRIKE NOW!'
            : boss.health <= boss.maxHealth / 2
              ? getLevelConfig().boss.name + ' · ENRAGED'
              : getLevelConfig().boss.name;
  ...
  function updateEnemies(dt) {
    ...
    for (const enemy of enemies) {
      if (enemy.support && enemy.onGround) {
        ...
        if (p) {
          enemy.x += p.x - enemy.support.x;
          enemy.y += p.y - enemy.support.y;
        }
      }
      enemy.animTime += dt;
      ...
      const inAwarenessWindow =
        enemy.x + enemy.w >= viewLeft && enemy.x <= viewRight && Math.abs(enemy.x - player.x) < 850;
      ...
      if (enemy.onGround && support) {
        ...
        if (needsRouteDecision) {
          ...
          const candidates = platforms
            .filter((p) => p !== support && p.y >= support.y - 210 && p.y <= support.y + 560)
            .map((p) => {
              const discriminant = launch * launch + 2 * gravity * (p.y - support.y);
              const flight = (launch + Math.sqrt(Math.max(0, discriminant))) / gravity;
              const landing = p.moving
                ? getPlatforms(worldTime + flight).find((next) => next.id === p.id)
                : p;
              const landingX = Math.max(
                landing.x + 22,
                Math.min(landing.x + landing.w - enemy.w - 22, player.x),
              );
              const velocity = (landingX - enemy.x) / flight;
              const score =
                Math.abs(landingX - player.x) + Math.abs(p.y - (player.y + player.h + 16)) * 1.2;
      ...
      if (enemy.onGround && enemy.vx && !enemy.boss) {
        const crowded = enemies.some(
          (other) =>
            other !== enemy &&
            other.health > 0 &&
            Math.abs(other.y - enemy.y) < 40 &&
            (other.x - enemy.x) * direction > 0 &&
            (other.x - enemy.x) * direction < enemy.w + 14,
        );
        if (crowded && distance > desiredGap + 45) enemy.vx *= 0.35;
      }
      ...
      for (const plat of platforms) {
        if (enemy.x + enemy.w > plat.x && enemy.x < plat.x + plat.w) {
          ...
          if (
            enemy.vy >= 0 &&
            enemyBottom >= plat.y &&
            enemyBottom <= plat.y + plat.h + enemy.vy * dt + 8
          ) {
  ...
  function updatePlayer(dt) {
    ...
    for (const type of Object.keys(attackCooldowns))
      attackCooldowns[type] = Math.max(0, attackCooldowns[type] - dt);
    ...
    player.support = null;
    if (
      getLevelConfig().boss?.enabled &&
      boss &&
      boss.health > 0 &&
      player.x > getLevelLength() - 900
    ) {
      boss.awakened = true;
    ...
    if (boss && boss.awakened && boss.health > 0)
      player.x = Math.max(getLevelLength() - 900, Math.min(getLevelLength() - 220, player.x));

    if (player.vy > 100) {
      const stompTarget = enemies.find(
        (e) =>
          !e.boss &&
          e.health > 0 &&
          e.defeatTimer <= 0 &&
          e.hurtTimer <= 0 &&
          player.x + player.w - 10 > e.x &&
          player.x + 10 < e.x + e.w &&
          previousFeet <= e.y + 24 &&
          player.y + player.h + 16 >= e.y + 16,
      );
      if (stompTarget) {
    ...
    for (const plat of platforms) {
      ...
      if (playerRight > plat.x && player.x < plat.x + plat.w) {
        ...
        if (
          player.vy >= 0 &&
          playerBottom >= plat.y &&
          playerBottom <= plat.y + plat.h + player.vy * dt + 8
        ) {
          player.y = plat.y - ph;
          player.vy = 0;
          player.onGround = true;
    ...
    if (
      player.y > Number(getLevelConfig().groundY ?? 1080) + 140 ||
      (!player.onGround &&
        player.vy >= 0 &&
        isOverGroundHole(player.x, player.w) &&
        player.y + ph >= Number(getLevelConfig().groundY ?? 1080) - 10)
    ) {
      handlePlayerFall();
  ...
  canvas.addEventListener('click', (event) => {
    ...
    parent.postMessage(
      {
        type: 'game-gift:point',
        x: ((event.clientX - r.left) / r.width) * viewportWidth() + cameraX,
        y: ((event.clientY - r.top) / r.height) * viewportHeight(),
      },
      location.origin,
    );
  });
...
window.addEventListener('message', (event) => {
  if (
    ...
    event.data?.type !== 'game-gift:init' ||
    started
  )
    return;
  started = true;
